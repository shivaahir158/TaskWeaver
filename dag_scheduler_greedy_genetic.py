...
- load imbalance across processors/resources
- memory-access bottlenecks or bandwidth limitations (if visible)

For every major bottleneck:
- explain why it hurts schedule quality
- explain whether it affects latency, throughput, utilization, or timing closure
- explain whether the bottleneck originates from dependencies, scheduling choices, or hardware resource limits
...
- whether communication costs or synchronization delays are limiting concurrency
- whether task placement across processors appears balanced

When discussing parallelism:
- use the provided independent_parallel_groups
- never violate DAG precedence constraints
- never claim dependent operations can execute simultaneously
- clearly distinguish theoretical parallelism from resource-constrained parallelism

PIPELINING, RETIMING, AND REGISTER/MEMORY INSERTION SECTION

Explain how scheduling and timing can be improved through pipelining, retiming, buffering, and memory insertion.
...
- whether pipeline stages should split long operations, long combinational paths, or critical dependency chains
- whether retiming opportunities exist
- whether buffering can reduce communication bottlenecks
- whether memory insertion can decouple stages and improve throughput
- whether pipeline balancing would improve stage utilization
- whether long dependency chains should be broken into multiple pipeline stages

Retiming guidance:
- discuss whether registers can be moved across operations or boundaries to improve timing balance
- explain how retiming may reduce critical-path delay
- explain how retiming may improve clock frequency
- explain whether retiming can improve stage balance
- mention assumptions required for legal retiming
- only recommend retiming where timing imbalance or long combinational regions appear likely
- preserve functional correctness and dependency ordering

Pipeline guidance:
- explain that registers/FIFOs/buffers mainly help by:
  - breaking long combinational paths
  - improving timing closure
  - increasing achievable clock frequency
  - balancing pipeline stages
  - improving throughput
  - reducing critical-path delay

Important:
- do NOT claim that register insertion allows a consumer operation to start before producer data becomes available
- consumer operations must still respect producer-consumer dependencies
- register insertion may improve clock frequency and throughput but may not reduce single-instance DAG latency
- avoid recommending registers on every incoming edge of a join node
- prefer only critical-path, high-delay, high-gap, communication-heavy, or timing-sensitive edges
- if suggesting decomposition or internal pipelining of an operation, explicitly state:

"This assumes the operation can be decomposed or internally pipelined; the DAG abstraction alone does not prove this."

- if operation internals are unknown, clearly state that the recommendation is a hypothesis
...
Requirements:
- provide edge-level recommendations using operation IDs
- rank recommendations from highest to lowest impact
- explain why each location helps
- distinguish:
  - register insertion
  - FIFO insertion
  - buffer insertion
...
- identify which recommendation primarily improves:
  - timing
  - throughput
  - utilization
  - latency
...
Latency:
- identify actions most likely to reduce makespan
- identify critical-path optimizations
- discuss scheduling improvements

Area:
- identify opportunities for resource sharing
- identify where duplication may be justified
- discuss hardware cost implications

Power:
- discuss switching activity implications
- discuss resource utilization implications
- discuss memory/register overhead
- discuss whether additional resources increase power

Throughput:
- discuss pipelining opportunities
- discuss stage balancing
- discuss concurrency improvements
- discuss utilization improvements
...
- explain tradeoffs between:
  ...
  - buffering
  - memory insertion
- explain when area growth is justified by latency or throughput gains
...
- Do NOT claim that registers or memory allow a dependent consumer operation to start before producer data is available.
- Distinguish scheduling improvements from timing improvements.
- Distinguish throughput improvements from latency improvements.
- Explicitly state assumptions whenever operation internals, RTL structure, memory architecture, resource types, communication fabric, timing slack, or implementation details are unknown.
- Focus on scheduling-aware hardware optimization rather than generic FPGA advice.
- Keep the answer concise, technically meaningful, and actionable for an HLS designer.
